  ...
  return (
    <section className="relative bg-midnight overflow-hidden px-5">
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center pt-24 md:pt-28 pb-10 md:pb-14">
        ...
        <p className="font-accent italic text-[clamp(17px,2.2vw,26px)] text-white/60 leading-snug max-w-lg mx-auto mb-1.5">{t(HERO.headline_accent.en, HERO.headline_accent.es)}</p>
        <p className="text-white/50 text-[13px] leading-relaxed max-w-sm mx-auto">{t(HERO.sub.en, HERO.sub.es)}</p>
        ...
        <div className="mt-3 text-[10px] text-white/20 tracking-wide">{SITE.brokerage} · {t("License", "Licencia")} #{SITE.license}</div>
      </div>
    </section>
  );
}
...
function StatsTicker() {
  ...
  return (
    <section className="bg-midnight border-y border-oro/10 py-4 overflow-hidden">
      <Marquee speed={18}>
        {STATS.map((s, i) => (<div key={i} className="flex items-center gap-3 mx-8 md:mx-10"><span className="text-oro font-extrabold text-[18px] md:text-[20px] tracking-tight">{s.value}</span><span className="text-white/40 text-[11px] md:text-[12px] font-medium">{t(s.en, s.es)}</span><span className="text-calma/20 mx-3 md:mx-4">◆</span></div>))}
        <div className="flex items-center gap-3 mx-8 md:mx-10"><span className="text-fuego font-extrabold text-[18px] md:text-[20px]">Mi Casa Dream</span><span className="text-white/40 text-[11px] md:text-[12px]">Pedro Giron · Salem, OR</span><span className="text-calma/20 mx-3 md:mx-4">◆</span></div>
      </Marquee>
    </section>
  );
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          <ScrollReveal>
            <div className="relative max-w-[420px]">
              ...
              <div className="absolute bottom-4 left-4 bg-midnight/90 backdrop-blur-sm border border-oro/15 rounded-lg px-4 py-2.5 z-10"><div className="font-display text-[13px] font-extrabold text-white tracking-tight">MI CASA <span className="text-fuego">DREAM</span></div><div className="flex items-center gap-1 mt-1"><span className="text-oro text-[10px]">★★★★★</span><span className="text-white/35 text-[9px]">{SITE.google.count} {t("reviews","reseñas")}</span></div></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-8">
        <ScrollReveal>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 mb-10">
            <div><div className="font-display text-[clamp(56px,9vw,110px)] font-extrabold text-white/[0.05] leading-none tracking-tighter select-none">MI CASA</div><div className="mt-3 pl-1"><div className="text-[11px] font-bold tracking-[0.12em] uppercase text-calma mb-2">{t("Home. Family. Roots.","Hogar. Familia. Raíces.")}</div><p className="text-white/55 text-[15px] leading-relaxed max-w-sm">{t("A home isn't just a building. It's where your kids grow up, where holidays happen, where you build something that's actually yours.","Una casa no es solo un edificio. Es donde crecen tus hijos, donde se celebran las fiestas, donde construyes algo que realmente es tuyo.")}</p></div></div>
            <div><div className="font-display text-[clamp(56px,9vw,110px)] font-extrabold text-fuego/[0.08] leading-none tracking-tighter select-none">DREAM</div><div className="mt-3 pl-1"><div className="text-[11px] font-bold tracking-[0.12em] uppercase text-oro mb-2">{t("Aspiration. Future. Possibility.","Aspiración. Futuro. Posibilidad.")}</div><p className="text-white/55 text-[15px] leading-relaxed max-w-sm">{t("Pedro started Mi Casa Dream because he believes every family in the Willamette Valley deserves someone who'll fight to make homeownership real. Not someday. Now.","Pedro creó Mi Casa Dream porque cree que cada familia en el Valle de Willamette merece a alguien que luche para hacer realidad el sueño de tener casa. No algún día. Ahora.")}</p></div></div>
          </div>
        </ScrollReveal>
    ...
    <section className="relative bg-midnight py-20 md:py-28 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 md:px-8 text-center">
        <ScrollReveal>
          ...
          <p className="text-white/50 text-[15px] mt-4 max-w-md mx-auto">{t("A free call is the first step. No commitment. No sales pitch. Just answers.","Una llamada gratis es el primer paso. Sin compromiso. Sin discurso de ventas. Solo respuestas.")}</p>
